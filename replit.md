...
- **Tools:** A comprehensive tool registry (`file_read`, `file_write`, `file_edit`, `file_search`, `shell_exec`, `web_browse`, `web_search`, `google_workspace`, `ennube_mcp`, `create_swarm`, `manage_swarm`, `server_restart`, `self_inspect`, `docs_read/write`, `x_post_tweet`, `x_delete_tweet`, `browser_control`, `kanban_comment`) with a permission layer and function calling integration for LLMs.
...
- **Kanban Board:** Project management board at `/kanban` with drag-and-drop columns and cards. Cards have priority levels, labels, and a comments/discussion section. Comments support `user`, `agent`, and `queen` author types. Agents and swarm queens can post comments via the `kanban_comment` tool. Data persists to `nami_kanban_columns`, `nami_kanban_cards`, and `nami_kanban_comments` PostgreSQL tables.
