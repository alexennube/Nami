...
import { dbGet, dbSet, getGoogleAccounts, upsertGoogleAccount, deleteGoogleAccount, setDefaultGoogleAccount, getDefaultGoogleAccount, dbSaveWorkspaceFile, dbDeleteWorkspaceFile, dbGetAuditLogs, dbGetAllAuditLogs } from "./db-persist";
...
export async function registerRoutes(
  ...
  const { kanbanDb, crmDb } = await import("./db-persist");
  const { registerKanbanRoutes } = await import("../modules/kanban/server/routes");
  const { registerCrmRoutes } = await import("../modules/crm/server/routes");
  const { createIntelligenceAnalyzer, startSequenceEngine: startSeqEngine, stopSequenceEngine: stopSeqEngine } = await import("../modules/crm/server/sequence-engine");

  const intelligenceAnalyzer = createIntelligenceAnalyzer({ log, executeToolCall, chatCompletion });

  registerKanbanRoutes(app, kanbanDb, logAudit);
  registerCrmRoutes(app, {
    db: crmDb,
    logAudit,
    log,
    runContactIntelligenceAnalysis: intelligenceAnalyzer,
    executeToolCall,
  });

  (globalThis as any).__startSequenceEngine = () => startSeqEngine({ db: crmDb, log, executeToolCall, chatCompletion, runContactIntelligenceAnalysis: intelligenceAnalyzer });
  (globalThis as any).__stopSequenceEngine = () => stopSeqEngine();
